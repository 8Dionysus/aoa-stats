#!/usr/bin/env python3
"""Compatibility entrypoint for the summary-catalog artifact-bundle mechanic."""
...
IMPL_PATH = (
    ...
    / "scripts"
    / "validate_abyss_machine_summary_catalog_bundle.py"
)
...
def _load_impl() -> ModuleType:
    ...
    spec = importlib.util.spec_from_file_location(
        "_aoa_stats_summary_catalog_artifact_bundle",
        IMPL_PATH,
    )
    if spec is None or spec.loader is None:
        raise RuntimeError(f"cannot load summary catalog bundle validator: {IMPL_PATH}")
