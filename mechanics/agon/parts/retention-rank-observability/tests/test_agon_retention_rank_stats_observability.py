...
def test_generated_registry_matches_seed():
    builder = _load(PART_ROOT / "scripts/build_agon_retention_rank_stats_observability_registry.py")
...
def test_validator_accepts_registry():
    validator = _load(PART_ROOT / "scripts/validate_agon_retention_rank_stats_observability.py")
    assert validator.main() == 0


def test_schemas_constrain_entry_and_registry():
    generated = json.loads((ROOT / "generated/agon_retention_rank_stats_observability_registry.min.json").read_text(encoding="utf-8"))
    entry_schema = json.loads((PART_ROOT / "schemas/agon-retention-rank-stats-observability.schema.json").read_text(encoding="utf-8"))
    registry_schema = json.loads((PART_ROOT / "schemas/agon-retention-rank-stats-observability-registry.schema.json").read_text(encoding="utf-8"))
