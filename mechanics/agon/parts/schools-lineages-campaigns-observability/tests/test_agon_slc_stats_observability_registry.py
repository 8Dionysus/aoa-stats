...
ENTRY_SCHEMA = PART_ROOT / 'schemas' / 'agon-slc-stats-observability.schema.json'
REGISTRY_SCHEMA = PART_ROOT / 'schemas' / 'agon-slc-stats-observability-registry.schema.json'
...
def test_builder_check_and_validator():
    assert subprocess.run([sys.executable, str(PART_ROOT / 'scripts' / 'build_agon_slc_stats_observability_registry.py'), '--check'], cwd=ROOT).returncode == 0
    assert subprocess.run([sys.executable, str(PART_ROOT / 'scripts' / 'validate_agon_slc_stats_observability_registry.py')], cwd=ROOT).returncode == 0
