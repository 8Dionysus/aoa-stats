...
def test_builder_check_and_validator():
    assert subprocess.run([sys.executable, str(PART_ROOT / 'scripts/build_agon_kag_stats_observability_registry.py'), '--check'], cwd=ROOT).returncode == 0
    assert subprocess.run([sys.executable, str(PART_ROOT / 'scripts/validate_agon_kag_stats_observability_registry.py')], cwd=ROOT).returncode == 0
