...
def load(name, rel):
    spec = importlib.util.spec_from_file_location(name, PART_ROOT / rel)
...
def test_registry_schema_requires_at_least_one_surface():
    schema = json.loads(
        (PART_ROOT / 'schemas/agon_mechanical_trial_stats_observability-registry.schema.json').read_text(
            encoding='utf-8'
        )
    )
