...
from build_agon_epistemic_stats_observability_registry import (  # noqa: E402
    BuildError,
    ITEM_KEY,
    OUT,
    REGISTRY_ID,
...
REQUIRED_FIELDS = ['candidate_outputs', 'forbidden_effects', 'live_protocol', 'observes', 'owner_repo', 'runtime_effect', 'status', 'summary_id']
...
ITEM_SCHEMA = PART_ROOT / "schemas" / "agon-epistemic-stats-observability.schema.json"
REGISTRY_SCHEMA = PART_ROOT / "schemas" / "agon-epistemic-stats-observability-registry.schema.json"
...
def validate_item(item: Any, item_validator: jsonschema.Draft202012Validator) -> str | None:
    ...
    if item.get("owner_repo") != "aoa-stats":
        return f"{key} owner_repo must be aoa-stats"
