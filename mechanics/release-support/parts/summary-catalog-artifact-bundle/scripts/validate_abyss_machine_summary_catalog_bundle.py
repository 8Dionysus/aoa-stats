...
DEFAULT_BUNDLE_DIR = REPO_ROOT / "dist" / "abyss-artifact-bundle" / "aoa-stats-summary-surface-catalog"
DEFAULT_REGISTRY_DIR = REPO_ROOT / "dist" / "abyss-artifact-registry" / "aoa-stats-summary-surface-catalog"
DEFAULT_SUBJECT_STORE_ROOT = REPO_ROOT / "dist" / "abyss-artifact-subjects" / "aoa-stats-summary-surface-catalog"
ARTIFACT_CLASS = "derived_observability_readmodel_catalog"
OWNER_REPO = "aoa-stats"
CONSUMER_INTENT = "agent"
CONSUMER_REF = "aoa-stats:summary-surface-catalog"
TRUST_ROOT_MODE = "host_managed"
PRODUCER = "aoa-stats summary surface catalog builder"
...
def _registry_roundtrip_with_subject_store(
    ...
    *,
    ...
    with _artifact_subject_store_roots(store_root, artifact_bundles):
...
def _trust_gate_pre_materialization_state(
    ...
    unexpected_blockers = sorted(
        set(str(blocker) for blocker in blockers) - {REQUIRED_SUBJECT_STORE_BLOCKER}
    )
    expected_deny = bool(
        ...
        and blockers == [REQUIRED_SUBJECT_STORE_BLOCKER]
        and inspected_claims.get("registry_latest", {}).get("selected_record_is_latest") is True
        and inspected_claims.get("controls", {}).get("required_controls_missing") == []
        and inspected_claims.get("source", {}).get("source_repo_matched") is True
        and inspected_claims.get("trust_root", {}).get("trust_root_mode_matched") is True
    ...
    return {
        ...
        "expected_pre_materialization_deny": expected_deny,
...
def _validate_in_bundle_dir(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(
        prefix="aoa-stats-pre-subject-store-",
...
def main() -> int:
    ...
    parser.add_argument(
        "--ephemeral",
        action="store_true",
        help="run the full roundtrip in a temporary directory instead of writing dist/",
    )
    ...
    if args.ephemeral:
        if args.no_clean:
            parser.error("--ephemeral cannot be combined with --no-clean")
        with tempfile.TemporaryDirectory(
            prefix="aoa-stats-summary-catalog-release-check-",
            dir=_default_tmp_root(),
        ) as tmp:
            tmp_root = Path(tmp)
